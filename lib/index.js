...
module.exports.compile = function(spec, _extraDefinitions, _encryptionDefinitions) {
  ...
  _.forEach(definitions, function (definition, key) {
    console.log('Loading schema: '+key);
    var object = null;
    ...
    if (object && !excludedSchema) {
      ...
      // add encryption to the schema object, if any
      if (_encryptionDefinitions) {
        var encryptedSchemas = _encryptionDefinitions.encryptedSchemas;
        if (encryptedSchemas.length > 0) {
          for (var i=0; i < encryptedSchemas.length; i++) {
            var encryptedSchema = encryptedSchemas[i];
            if (key === encryptedSchema.name) {
              console.log('..enable encryption: '+JSON.stringify(encryptedSchema));
              schema.plugin(encrypt, {
                encryptionKey: _encryptionDefinitions.enc,
                signingKey: _encryptionDefinitions.sig,
                encryptedFields: encryptedSchema.fields,
          ...
          console.warn('Warning: encrypted definitions provided but no schemas listed!');
        }
      }
      schemas[key] = schema
    }
